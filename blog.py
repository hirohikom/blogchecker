    ...
    try:
        ...
        if body_area:
            ...
            if client and len(clean_text) > 50:
                prompt = f"""以下のアイドルブログの本文を読み、魅力を損なわない形で、本文の引用を含めながら少し詳しめに抄訳してください。

                【ルール】
                1. 端折りすぎず、具体的なエピソードや情景や感情がしっかり伝わる充実した文章量にしてください。
                2. 語尾は必ず敬体（「〜です」「〜ます」「〜とのことです」など）で統一してください。
                3. 文頭の指定：書き出しの冒頭にメンバー名（「〇〇は〜」）や挨拶を置かず、ブログ内の出来事やエピソードから直接書き出してください。
                4. 視点：一人称の主体者視点（「〜します」）ではなく、第三者視点の客観的な表現（「〜しています」「〜と語っています」）にしてください。
                5. 本文からの引用部分は必ずカギ括弧「」で囲んでください。
                6. 執筆者本人を指す三人称は「彼」ではなく「彼女」を使ってください。
                7. 「僕青」という単語は「僕」を省略せずそのまま「僕青」と表記してください。
                8. 感情表現や独特な言い回し、文脈が飛んでいて意味の解釈が難しい文章は、無理に要約・補正しようとせず、原文のままカギ括弧「」で引用してください。
                ...
                try:
                    response = client.chat.completions.create(
                        model="gpt-4o-mini",
                        messages=[
                            {"role": "system", "content": "あなたはアイドルブログの内容を魅力的に紹介する優秀なライターです。"},
