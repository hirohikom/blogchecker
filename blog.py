    ...
    try:
        ...
        if body_area:
            ...
            if client and len(clean_text) > 50:
                prompt = (
                    f"以下のアイドルブログの本文を読み、魅力を損なわない形で、本文の引用を含めながら、少し詳しめに抄訳してください。\n\n"
                    f"【ルール】\n"
                    f"1. あまり約しすぎず、最低でも元のブログの文章の半分くらいの長さになるようにしてください。\n"
                    f"2. 語尾は必ず敬体（「〜ます」「〜です」など）で統一してください。\n"
                    f"3. 誰のブログかは既に明らかなため、最初の一文の冒頭に「〇〇（メンバー名）は〜」という主語は書かないでください。\n"
                    f"4. 「～します」というような主体者視点の表現ではなく、「～しています」というような客観視点の表現にしてください。\n\n"
                    f"5. 本文の引用部分は必ずカギ括弧で囲ってください。\n"
                    f"【本文】\n{clean_text}"
                )
